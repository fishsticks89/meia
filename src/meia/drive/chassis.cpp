...
namespace meia {
    //! Motor Functions
    void Chassis::set_voltage(std::pair<int, int> voltages) {
    ...
    Chassis::get_motor_temps() {
        ...
        for (auto motor : left_motors) {
            l_temps.push_back(pros::c::motor_get_temperature(abs(motor)));
        }
        for (auto motor : right_motors) {
            r_temps.push_back(pros::c::motor_get_temperature(abs(motor)));
        }
    ...
    // Brake modes
    void Chassis::set_drive_brake(pros::motor_brake_mode_e_t input) {
        for (int i : left_motors) {
    ...
    double curve_function(int x, double scale) {
        return (scale != 0 && x != 100) ? (powf(2.718, -(scale / 10)) + powf(2.718, (abs(x) - 127) / 10) * (1 - powf(2.718, -(scale / 10)))) * x : x;
    }
